  ...
  3: [
    {
      ...
      content:
        "Objective: Introduce key themes and language in Shakespeare's works.",
    },
  ],
};
...
export default function App() {
  ...
  const [isDeleteLessonPlanModalOpen, setDeleteLessonPlanModalOpen] =
    useState(false);
  const [lessonPlanToDelete, setLessonPlanToDelete] = useState(null);
  const [isUploadAccommodationModalOpen, setUploadAccommodationModalOpen] =
    useState(false);
  const [pendingFile, setPendingFile] = useState(null);
  const [isUploadLessonPlanModalOpen, setUploadLessonPlanModalOpen] =
    useState(false);
  const [isRenameAndUploadModalOpen, setRenameAndUploadModalOpen] =
    useState(false);
            ...
            (querySnapshot) => {
              ...
              if (querySnapshot.empty) {
                ...
                initialClasses.forEach((c) => {
                  ...
                  plans.forEach((lp) => {
                    const lpDocRef = doc(
                      collection(classDocRef, "lessonPlans"),
                      lp.id
                    );
                    batch.set(lpDocRef, {
                      name: lp.name,
                ...
                batch
                  .commit()
                  .catch((e) =>
                    console.error("Error writing initial batch: ", e)
                  );
              } else {
      ...
      onAuthStateChanged(auth, async (user) => {
        if (user) {
          ...
          signInAnonymously(auth)
            .then((cred) => authAndLoad(cred.user))
            .catch((e) => {
              console.error("Anonymous sign-in error", e);
              if (e.code === "auth/configuration-not-found") {
                setError(
                  "Connection failed: Anonymous sign-in is not enabled. Please go to your Firebase project console, navigate to Authentication > Sign-in method, and enable the 'Anonymous' provider."
                );
              } else {
  ...
  const selectedLessonPlan = lessonPlans.find(
    (lp) => lp.id === selectedLessonPlanId
  );
  ...
  useEffect(() => {
    if (selectedClassId && db && userId) {
      const lessonPlansCollectionRef = collection(
        db,
        "artifacts",
        appId,
        "users",
  ...
  useEffect(() => {
    if (
      ...
      (!selectedLessonPlanId ||
        !lessonPlans.some((lp) => lp.id === selectedLessonPlanId))
    ) {
  ...
  const handleAddNewClass = async () => {
    if (newClassName.trim() && db && userId) {
      try {
        ...
        const newClassRef = await addDoc(classesCollectionRef, {
          name: newClassName.trim(),
          accommodations: "",
        });
        const lessonPlansRef = collection(newClassRef, "lessonPlans");
        await addDoc(lessonPlansRef, {
        ...
        showTempNotification("Error creating class.", true);
        console.error(e);
      }
    }
  };
  ...
  const confirmDeleteClass = async () => {
    if (classToDelete && db && userId) {
      try {
        ...
        showTempNotification("Error deleting class.", true);
        console.error(e);
      }
    }
  };
  ...
  const handleAddNewLessonPlan = async () => {
    if (newLessonPlanName.trim() && selectedClassId && db && userId) {
      try {
        const lessonPlansRef = collection(
          db,
          "artifacts",
          appId,
          "users",
        ...
        const newDoc = await addDoc(lessonPlansRef, {
        ...
        showTempNotification(
          `Lesson plan "${newLessonPlanName.trim()}" created!`
        );
        setNewLessonPlanName("");
        setAddLessonPlanModalOpen(false);
      } catch (e) {
        showTempNotification("Error creating lesson plan.", true);
        console.error(e);
      }
    }
  };
  ...
  const confirmDeleteLessonPlan = async () => {
    if (lessonPlanToDelete) {
      try {
        ...
        showTempNotification("Error deleting lesson plan.", true);
        console.error(e);
      }
    }
  };
  ...
  const handleAccommodationChange = (text) => {
    setClasses(
      classes.map((c) =>
        c.id === selectedClassId ? { ...c, accommodations: text } : c
      )
    );
    setSaveStatus("saving");
    if (accommodationChangeTimeout.current)
  ...
  const handleLessonPlanContentChange = (text) => {
    ...
    setSaveStatus("saving");
    if (lessonPlanChangeTimeout.current)
    ...
    lessonPlanChangeTimeout.current = setTimeout(async () => {
      if (db && userId && selectedClassId && selectedLessonPlanId) {
        ...
        try {
          ...
          console.error("Autosave error for lesson plan:", e);
          setSaveStatus("idle");
        }
      }
    }, 750);
  ...
  const handleFileUpload = (file, fileType) => {
    if (fileType === "accommodations") {
      ...
      if (!selectedLessonPlan) {
        showTempNotification(
          "Please select a lesson plan to add content to.",
          true
        );
        return;
      }
  ...
  const processFileUpload = async (file, mode) => {
    ...
    try {
      const response = await fetch("https://iep-harmony-backend.onrender.com", {
        method: "POST",
        body: formData,
      });
      if (!response.ok) {
      ...
      if (mode === "replace" || mode === "merge") {
        const current =
          mode === "merge" ? selectedClass?.accommodations || "" : "";
      ...
      showTempNotification(
        `Could not process file. Is the backend server running?`,
        true
      );
    }
  };
  ...
  const handleAnalyze = async () => {
    ...
    const accommodationsToAnalyze = selectedClass.accommodations
      .split("\n")
      .filter((a) => a.trim() !== "");
    if (accommodationsToAnalyze.length === 0) {
      showTempNotification(
        "The selected class has no accommodations listed.",
        true
      );
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans text-gray-800">
      {notification && (
        <div className="fixed top-5 right-5 bg-blue-500 text-white py-2 px-4 rounded-lg shadow-lg animate-fade-in-out z-50">
          {notification}
        </div>
      )}
      {error && (
        <div className="fixed top-5 right-5 bg-red-500 text-white py-2 px-4 rounded-lg shadow-lg animate-fade-in-out z-50">
          {error}
        </div>
      )}
          ...
          <div className="lg-col-span-1 space-y-6">
            <div className="bg-white p-5 rounded-xl shadow-sm">
              <h2 className="text-lg font-semibold flex items-center mb-4">
                <ListChecks className="mr-2 text-indigo-500" />
              ...
              <div className="space-y-3">
                <div className="flex items-center space-x-2">
                  ...
                  <button
                    onClick={() => openDeleteClassModal(selectedClassId)}
                    disabled={!selectedClass}
                    className="p-2 bg-red-500 text-white rounded-md hover:bg-red-600 disabled:bg-gray-300"
                  >
                    <Trash2 size={20} />
                  </button>
                </div>
                {selectedClass ? (
                  <div className="pt-4 border-t">
                    ...
                    <p className="text-sm text-gray-600 mb-3">
                      Paste or edit accommodations for{" "}
            ...
            <div className="bg-white p-5 rounded-xl shadow-sm">
              <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <button
                    onClick={openDeleteLessonPlanModal}
              ...
              <textarea
                className="w-full h-64 p-4 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500 text-sm leading-6 bg-white"
                placeholder={
                  selectedClass
                    ? "Select or create a lesson plan to begin."
              ...
              <div className="mt-4 flex justify-end">
                <button
                  ...
                  disabled={isLoading || !lessonPlanContent || !selectedClass}
                  className="flex items-center justify-center bg-green-600 text-white font-bold py-3 px-6 rounded-lg hover:bg-green-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition-all duration-200"
                >
      ...
      >
        ...
        <input
          ...
          placeholder="e.g., Period 5 - Geometry"
      ...
      <Modal
        isOpen={isAddLessonPlanModalOpen}
        onClose={() => setAddLessonPlanModalOpen(false)}
      >
        ...
        <input
          ...
          placeholder="e.g., Unit 1: The Odyssey"
          className="w-full p-2 border border-gray-300 rounded-md mb-4"
          autoFocus
        />
        <div className="flex justify-end space-x-2">
          ...
          <button
            onClick={handleAddNewLessonPlan}
      ...
      <Modal
        isOpen={isDeleteLessonPlanModalOpen}
        onClose={() => setDeleteLessonPlanModalOpen(false)}
      >
        <h3 className="text-lg font-semibold mb-2">Confirm Deletion</h3>
        <p className="text-gray-600 mb-4">
          Are you sure you want to delete the lesson plan "
          {lessonPlanToDelete?.name}"? This action cannot be undone.
        </p>
        <div className="flex justify-end space-x-2">
          <button
          ...
          <button
            onClick={confirmDeleteLessonPlan}
            className="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700"
          >
            Delete
          </button>
      ...
      <Modal
        isOpen={isUploadAccommodationModalOpen}
        onClose={() => setUploadAccommodationModalOpen(false)}
      >
        <h3 className="text-lg font-semibold mb-4">Update Accommodations</h3>
        <p className="text-gray-600 mb-4">
          How would you like to add the accommodations from the uploaded file?
        </p>
        <div className="flex justify-end space-x-2">
          <button
            onClick={() => handleConfirmAccommodationUpload("replace")}
            className="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700"
          >
            Replace Existing
          </button>
          <button
            onClick={() => handleConfirmAccommodationUpload("merge")}
            className="px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700"
          >
            Add to Existing
          </button>
      ...
      <Modal
        isOpen={isUploadLessonPlanModalOpen}
        onClose={() => setUploadLessonPlanModalOpen(false)}
      >
        <h3 className="text-lg font-semibold mb-4">
          Update Lesson Plan Content
        </h3>
        <p className="text-gray-600 mb-4">
          How would you like to add the content from the uploaded file to the
          current lesson plan?
        </p>
        <div className="flex justify-end space-x-2">
          <button
            onClick={() => handleConfirmLessonPlanUpload("replace-lesson")}
            className="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700"
          >
            Replace Existing
          </button>
          <button
            onClick={() => handleConfirmLessonPlanUpload("merge-lesson")}
            className="px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700"
          >
            Add to Existing
          </button>
      ...
      <Modal
        isOpen={isRenameAndUploadModalOpen}
        onClose={() => setRenameAndUploadModalOpen(false)}
      >
        ...
        <div className="flex justify-end space-x-2">
          ...
          <button
            onClick={handleConfirmRenameAndUpload}
            className="px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700"
          >
            Save Title and Upload
          </button>
        </div>
      </Modal>
    </div>
...
function AnalysisItem({ item }) {
  ...
  const StatusIcon = ({ status }) => {
    ...
    if (status === "Partially Met")
      return <AlertCircle className="text-yellow-500 flex-shrink-0" />;
      ...
      {item.suggestion && (
        <div className="pl-8 mt-2">
          ...
          >
            ...
            {isSuggestionVisible
              ? "Hide Suggestion"
              : "Show Suggestion to Meet Requirement"}
          </button>
          {isSuggestionVisible && (
